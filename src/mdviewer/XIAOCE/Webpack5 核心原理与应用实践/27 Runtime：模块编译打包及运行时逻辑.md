...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3276ff5d31014f7d913f07b6bc61e4ab~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/72e8b113bab844e9acf2e5d682339590~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
在上一章《**Chunk：三种产物的打包逻辑**》中，我们已经介绍了 `compilation.seal` 函数内会调用 `buildChunkGraph` 生成 `Chunk` 依赖关系图，之后 Webpack 就可以分析出：
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6a5888bf286248adaf7b627310aebbd9~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f6d0eb4a736b493b86fc8c6945f8534a~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/683c91eefab04caf8875b859a7f7910c~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
上面我们聊到，除直接操作 `source` 外，`Template.apply` 中还可能通过 [initFragments](https://github1s.com/webpack/webpack/blob/HEAD/lib/InitFragment.js) 数组达成修改模块产物的效果。`initFragments` 数组项为 `InitFragment` 子类实例，它们带有两个关键函数：`getContent`、`getEndContent`，分别用于获取代码片段的头尾部分。
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e9bb7223e90c4054bb50f95c74191dd8~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/df188ee10e674b068e4bd4ea654aa923~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/098592e677784728ba6c4947235b0460~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
[$\color {#0269c8} {第一次循环：收集模块依赖：}$](https://github1s.com/webpack/webpack/blob/HEAD/lib/Compilation.js#L3392-L3393)

在上述「模块转译主流程」中，我们聊到 `Template.apply` 函数可能修改模块的 `runtimeRequirements` 数组，最终形成如下结构：

![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4ced49e0b3764a4aaf168db7bae8c1cb~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
[$\color {#0269c8} {第二次循环：整合 chunk 依赖：}$](https://github1s.com/webpack/webpack/blob/HEAD/lib/Compilation.js#L3463-L3464)

第一次循环针对 module 收集依赖，第二次循环则遍历 chunk 数组，收集将其对应所有 module 的 runtime 依赖，例如：

![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f1d02df6676942acac39290096af3d4c~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

示例图中，`module a` 包含两个运行时依赖；`module b` 包含一个运行时依赖，则经过第二次循环整合后，对应的 `chunk` 会包含两个模块所包含的三个运行时依赖。

[$\color {#0269c8} {第三次循环：依赖标识转 RuntimeModule 对象：}$](https://github1s.com/webpack/webpack/blob/HEAD/lib/Compilation.js#L3483-L3484)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/67a464acdc17450487c13f649c7d9b4f~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
模块合并打包过程会将 chunk 对应的 module 及 runtimeModule 按规则塞进**模板框架**中，最终合并输出成完整的 bundle 文件，例如上例中：

![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/37ce17c38d524705b0f4dfbb7f408782~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
