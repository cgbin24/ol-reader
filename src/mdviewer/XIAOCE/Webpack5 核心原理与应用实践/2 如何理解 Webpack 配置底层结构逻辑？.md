...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/784ef02a144a4b3ebfbcbe13b06ac7bb~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/02ccea1194e045689143011ef62ff553~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
[Vue CLI](https://cli.vuejs.org/guide/) 全称 Vue.js Command-Line Interface，是由 Vue 核心团队开发，用于帮助用户快速创建、运行 Vue.js 项目脚手架的命令行工具。实现上，[Vue CLI](https://cli.vuejs.org/guide/) 底层调用 Webpack 实现针对 `.vue` 等资源的编译打包功能；调用 webpack-dev-server 实现包含 HMR 功能的开发服务器功能；还能通过插件方式整合 ESLint、Babal、Less 等工具。

总之， [Vue CLI](https://cli.vuejs.org/guide/) 提供了全套开箱即用的 Vue 项目开发、调试环境，开发者可以跳过繁琐复杂的环境配置过程，而专注于业务代码开发上。

与其它工具类似，使用 [Vue CLI](https://cli.vuejs.org/guide/) 时首先需要安装依赖：
```sh
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d212c315f0f44ca49178e6e9a8c695c9~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5e0f2b01c9864747a63f212c6fee355e~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

> 提示：具体用法，请参考 [Vue CLI](https://cli.vuejs.org/guide/) 官网：[cli.vuejs.org/](https://cli.vuejs.org/)

经过上述步骤后，[Vue CLI](https://cli.vuejs.org/guide/) 最终都会生成对应的文件并自动安装依赖，之后就可以使用 `serve` 命令启动项目：
```sh
npm run serve
# 或者
yarn serve
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/040c12736c64414f9c9390ea31eaae59~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)

[Vue CLI](https://cli.vuejs.org/guide/) 底层依赖于 Webpack 实现编译打包等工程化能力，开发者可通过 configureWebpack 与 chainWebpack 配置项修改 Webpack 配置信息。
...
`configureWebpack` 的配置规则与 Webpack 一致，同样支持 `plugins/module/resolve` 等配置项。实际上，[Vue CLI](https://cli.vuejs.org/guide/) 内部最终会调用 [webpack-merge](https://github.com/survivejs/webpack-merge) 将 `configureWebpack` 值与其它上下文配置合并，生成最终的 `Webpack` 配置信息。
...
> 提示：更多信息可参考 [Vue CLI](https://cli.vuejs.org/guide/) 官网 [Working with Webpack](https://cli.vuejs.org/guide/webpack.html#simple-configuration) 一节。

[Vue CLI](https://cli.vuejs.org/guide/) 在 Webpack 基础上包装多一层更易用的功能结构，这确实能极速提升研发效率与体验，但代价则是我们完全不清楚其内部运作细节，这会导致开发者比较难做一些深度定制或者性能优化，此时可使用 `inspect` 命令生成完整的 Webpack 配置信息：
```lua
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dbe1cb77ffa0492cb32d5236cc0e22f0~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
更多用法，可查阅帮助文档：`vue inspect --hel`p。

### 使用 [CRA](https://reactjs.org/docs/create-a-new-react-app.html) 搭建项目脚手架

[CRA](https://reactjs.org/docs/create-a-new-react-app.html) 全称 Create React Application，同样是用于帮助用户快速创建、运行 React 项目脚手架的命令行工具，功能、底层实现、用法都与 [Vue CLI](https://cli.vuejs.org/guide/) 相似，都提供了全套开箱即用的 React 项目开发、调试环境。
...
> 提示：更多用法，请参考 [CRA 官网](https://github.com/facebook/create-react-app)： [github.com/facebook/cr…](https://github.com/facebook/create-react-app%E3%80%82)
