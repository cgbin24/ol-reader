...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ecc54367df754726addbebc84c708e8a~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0d64b4bd8375473eb22e94311e84b9e0~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
读到这里，一些同学可能会有这样的疑虑：这里修言敢说 `compose()` 是“两两组合、循环往复”的过程，是因为在**第14节**实现 `compose()` 函数的时候，修言【故意】使用了具备二元运算回调的 `reduce()`来实现 `compose()`，这才使得两个章节的逻辑互洽了——哈，修勾果然鸡贼！
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/93a8bc31891e4b84bc9191185a92106e~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4d045ba285534effbabf1e1a9d18efaa~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp)
...
其实，`compose()`的单位元函数，咱们早在**第18节**就见过啦。没错，我说的就是 `Identity Function`（恒等函数）：
```js
const identity = x => x
```
...
这也使我忍不住联想起了前几年在社区听到过的一种声音——“函数组合是一种 Monoid”。那之后我查了很多的资料，始终也没能为这个观点找到什么强有力的学术理论支撑。因此，上文的推导，并不是为了佐证这个观点，而是为了凸显我们在**第17节**就阐述过的一个思路——**范畴的本质是复合**。
