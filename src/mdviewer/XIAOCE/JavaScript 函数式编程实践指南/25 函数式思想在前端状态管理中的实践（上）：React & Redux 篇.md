...
> “不可变数据”的内涵我们在**第 6 节-第 10 节**已经有过非常深入的探讨，此处不再赘述。
...
通过**第 23 节**的学习，我们已经知道，**React 组件是一个吃进数据、吐出 UI 的【纯函数】。**
...
> 注：柯里化是把 **1 个 n 元函数**改造为 **n 个相互嵌套的一元函数**的过程，它是高阶函数的一种应用，详见**小册 15 节**。
...
我们来看最关键的一行，也就是最后一行，这行代码和我们 **第 14 节** 实现的版本不同，但和我们**第 22 节**实现的 pipe函数可是亲兄弟呀：
```js
// 第 22 节实现的 pipe 函数
